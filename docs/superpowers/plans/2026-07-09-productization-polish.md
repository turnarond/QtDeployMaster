...
**Tech Stack:** CMake 3.16 + MSVC(rc.exe 资源编译)、Qt 6.11.1(Qt6::EntryPoint 处理 WIN32 入口 + QIcon)、Windows VERSIONINFO、Python 3.14 / PowerShell(.ico 生成)。
...
- 环境：验证运行需 `export PATH="/c/Qt/6.11.1/msvc2022_64/bin:$PATH"` 且输出目录有 `libcurl.dll`（POST_BUILD 已自动复制）。
...
Run:
```bash
cd /d/persional/QtDeployMaster/build && cmake .. -DCMAKE_PREFIX_PATH="C:/Qt/6.11.1/msvc2022_64" >/tmp/cfg.log 2>&1 && cmake --build . --config Release 2>&1 | tail -4
```
...
Run:
```bash
cd /d/persional/QtDeployMaster/build/Release && export PATH="/c/Qt/6.11.1/msvc2022_64/bin:$PATH" && ./DeviceForge.exe & PID=$!; sleep 4; kill -0 $PID 2>/dev/null && echo "ALIVE 无console" && taskkill //F //PID $PID 2>/dev/null || echo "EXITED"
```
...
```bash
cd /d/persional/QtDeployMaster/build && cmake --build . --config Release 2>&1 | tail -3
cd Release && export PATH="/c/Qt/6.11.1/msvc2022_64/bin:$PATH" && ./DeviceForge.exe & PID=$!; sleep 4; kill -0 $PID 2>/dev/null && echo "ALIVE" && taskkill //F //PID $PID 2>/dev/null || echo "EXITED"
```
