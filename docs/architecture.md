...
NetRelayTool 是一个透明中继代理：数据生产者（TCP/UDP 客户端）连接工具，工具**原封不动**地双向转发到真实消费者，同时旁路抓取流量用于调试，并可录制流量以便日后回放。
...
| 单元 | 职责 |
|------|------|
| `NetRelayBackend` | 中继引擎（TCP 配对代理 / UDP 会话代理）+ 录制钩子 + 回放委托 + `RelayMode{Idle,Relaying,Replaying}` 互斥状态机 |
