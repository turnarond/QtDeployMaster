...
**Tech Stack:** C++17、Qt 6.11.1 (QUdpSocket 组播 API / QNetworkInterface)、QDataStream(LittleEndian)、QtTest + CTest。
...
- 环境：跑测试/程序前 `export PATH="/c/Qt/6.11.1/msvc2022_64/bin:$PATH"`；输出目录需 `libcurl.dll`（POST_BUILD 自动复制）。
...
Run: `cd /d/persional/QtDeployMaster/build && cmake --build . --config Release --target tst_nrec && export PATH="/c/Qt/6.11.1/msvc2022_64/bin:$PATH" && ./tests/Release/tst_nrec.exe -o /tmp/r.txt,txt; cat /tmp/r.txt | grep -E "PASS|FAIL"`
...
cd Release && export PATH="/c/Qt/6.11.1/msvc2022_64/bin:$PATH" && cp -f ../../lib/libcurl-x64.dll libcurl.dll 2>/dev/null; timeout 6 ./DeviceForge.exe >/dev/null 2>&1 & PID=$!; sleep 4; kill -0 $PID 2>/dev/null && echo ALIVE && kill $PID 2>/dev/null || echo EXITED
```
