...
uint32_t pb_get_decode_size(const pb_msgdesc_t *descriptor) 
{
    uint32_t word0;
    uint32_t data_offset_total = 0;
    int field_id = 0;

    int data_offset = 0;
    int data_size = 0;

    for (int i = 0 ; i < descriptor->largest_tag; i++)
    {
        word0 = PB_PROGMEM_READU32(descriptor->field_info[field_id]);
        switch(word0 & 3)
        {
        case 0: {
            /* 1-word format */
            data_offset = (word0 >> 16) & 0xFF;
            data_size = (word0 >> 28) & 0x0F;
            field_id += 1;
            break;
        }
        ...
        case 1: {
            /* 2-word format */
            uint32_t word1 = PB_PROGMEM_READU32(descriptor->field_info[field_id + 1]);
            int array_size = (word0 >> 16) & 0x0FFF;
            data_offset = word1 & 0xFFFF;
            
            data_size = (pb_size_t)((word1 >> 16) & 0x0FFF) * array_size;
            field_id += 2;
            break;
        }
        ...
        case 2: {
            /* 4-word format */
            uint32_t word2 = PB_PROGMEM_READU32(descriptor->field_info[field_id + 2]);
            uint32_t word3 = PB_PROGMEM_READU32(descriptor->field_info[field_id + 3]);

            int array_size = (word0 >> 16);
            data_offset = word2;
            data_size = word3 * array_size;
            field_id += 4;
            break;
        }
        ...
        data_offset_total = data_size + data_offset;
    }
    return data_offset_total;
}
