...
### 架构状态：DeployMaster 2.0 Phase 0-1 完成

项目已完成从 MVP+EventBus 单体架构到 **lwserverbase 服务核 + Qt Widget 壳** 双层架构的基础设施搭建。
...
**待完成（Phase 2-3）**：
- Phase 2（全量迁移）：TelnetTool、LogQueryTool、ModbusTool、WebSocketTool、DiagnosticTool
- Phase 3（插件化）：QPluginLoader DLL 加载、SSH Adapter/Terminal Tool、主窗口代码化
...
|----------|-------|----------|------|----------|
| 批量部署 | FtpDeployWidget (Tool) | FtpDeployBackend → FtpAdapter | FTP (libcurl) | 已迁移 Tool 架构 |
| 批量命令 | `TelnetDeploy` | `TelnetClient` | Telnet (QTcpSocket) | 旧模式，待迁移 |
...
| WebSocket | `WebSocketClient` | 内建 Server/Client 双模式 | WebSocket (QWebSocket) | 旧模式，待迁移 |
| 诊断分析 | `DiagnosticClient` | tinyvsoa + 解压引擎 | VSOA | 无法编译 |
...
**需求文档（doc/）**：
- `doc/Qt上位机诊断工具需求规格说明书.md` — 诊断工具 PRD（VSOA 远程诊断、一键日志采集、智能下载续传、主备对比、4 阶段 7 周路线图）
- `doc/qt上位机与PLC诊断服务通信技术规约.md` — VSOA 通信协议技术规约（URL 路由、JSON 参数格式、RPC 模式、文件分块策略）
...
- **tinyvsoa 目录缺失**：`CMakeLists.txt` 引用了 `tinyvsoa/tinyvsoa.cpp`，`DiagnosticClient.h` 引用了 `tinyvsoa/tinyvsoa.h`，但 `tinyvsoa/` 目录不在仓库中。诊断分析模块（DiagnosticClient）当前无法编译，需补充 VSOA 库源码
...
- **main.cpp 初始化顺序**（关键）：
  ...
  4. `ToolRegistry` 注册内置 Tool
  5. `ToolHost` 工厂注册
  6. `DeviceBusWidget` + 旧 UI 初始化
  7. 加载 darkstyle.qss
  8. `window.show()`
