...
| 模块 | 状态 | 协议 | 说明 |
|------|------|------|------|
| 批量文件部署 | ✅ 已迁移 Tool 架构 | FTP (libcurl) | 拖拽文件/文件夹，批量上传到多台设备 |
| 批量命令执行 | ⚠ 旧模式 | Telnet (QTcpSocket) | 向多台设备同时发送 Shell 命令 |
| 日志查询下载 | ⚠ 旧模式 | FTP (libcurl) | 远程日志检索与下载 |
| Modbus 集群测试 | ⚠ 旧模式 | Modbus TCP | 批量读写寄存器，自动刷新 |
| WebSocket 通信 | ⚠ 旧模式 | WebSocket | 内建 Server/Client 双模式 |
| OPC UA 客户端 | ⚠ 演示 | OPC UA | 硬编码演示数据，待实现 |
| VSOA 诊断分析 | ❌ 待 tinyvsoa 补全 | VSOA | 远程触发诊断采集、下载解压分析 |
